...
void onControlChange(Channel thisChannel, uint8_t controller, uint8_t value, Cable cable) {
     ...
     if(controller == 11) {
    if (expresschannel == 10) {
      cur_express1 = value;
    }
    if (expresschannel == 11) {
      cur_express2 = value;
    }
    if (expresschannel == 12) {
      cur_express3 = value;
    }
    if (expresschannel == 13) {
      cur_express4 = value;
    }
  }
}
void onNoteOn(Channel myChannel, uint8_t note, uint8_t velocity, Cable cable) {
  ...
  if(channel == 10 || channel == 11 || channel == 12 || channel == 13){
...
void loop() {
  ...
  if (pushbutton.getState() == Button::Rising) {
    usbMIDI.sendNoteOff(lastPitch3, lastVelocity3, 14);
    usbMIDI.sendNoteOn(myPitch1, myVelocity1, 14);
    usbMIDI.sendNoteOff(lastPitch1, lastVelocity1, 4);
    usbMIDI.sendNoteOn(myPitch1, myVelocity1, 4);
    lastPitch3 = myPitch1;
    lastVelocity3 = myVelocity1;
    usbMIDI.sendNoteOn(myPitch1, myVelocity1, 12);
    ...
    usbMIDI.sendNoteOff(lastPitch1, lastVelocity1, 14);
    usbMIDI.sendNoteOn(myPitch1, myVelocity1, 14);
    usbMIDI.sendNoteOff(lastPitch1, lastVelocity1, 4);
    usbMIDI.sendNoteOn(myPitch1, myVelocity1, 4);
    lastPitch3 = myPitch1;
    lastVelocity3 = myVelocity1;
    usbMIDI.sendNoteOff(lastPitch1, lastVelocity1, 12);
    usbMIDI.sendNoteOff(lastPitch1, lastVelocity1, 2);
    pressed1 = 0;
  }
  if (pushbutton1.getState() == Button::Rising) {
    usbMIDI.sendNoteOff(lastPitch, lastVelocity, 13);
    usbMIDI.sendNoteOn(myPitch, myVelocity, 13);
    usbMIDI.sendNoteOff(lastPitch, lastVelocity, 3);
    usbMIDI.sendNoteOn(myPitch, myVelocity, 3);
    lastPitch2 = myPitch;
    lastVelocity2 = myVelocity;
    usbMIDI.sendNoteOn(myPitch, myVelocity, 11);
    ...
    lastPitch2 = myPitch;
    lastVelocity2 = myVelocity;
    usbMIDI.sendNoteOff(lastPitch,lastVelocity, 11);
  ...
  if(pressed1 == 1 ) {
    ...
    if (myPitch == 73 && las_pitchbend4 != cur_pitchbend4 ) {
      usbMIDI.sendPitchBend(cur_pitchbend4, 13);
      las_pitchbend4 = cur_pitchbend4;
    }
    else if (myPitch == 66 && las_pitchbend3 != cur_pitchbend3 ) {
      usbMIDI.sendPitchBend(cur_pitchbend3, 13);
      las_pitchbend3 = cur_pitchbend3;
    }
    else if (myPitch == 59 && las_pitchbend2 != cur_pitchbend2 ) {
      usbMIDI.sendPitchBend(cur_pitchbend2, 13);
      las_pitchbend2 = cur_pitchbend2;
    }
    else if(myPitch == 52 && las_pitchbend1 != cur_pitchbend1){
      usbMIDI.sendPitchBend(cur_pitchbend1, 13);
      las_pitchbend1 = cur_pitchbend1;
    }
